...
func TestDetectIssueReturnsRepetitionDetail(t *testing.T) {
	...
	loop := strings.Repeat("the quick brown fox jumps over ", 5)
	issue, detail := DetectIssue(cfg, loop, "length", false)
...
func TestDetectIssueRepetitionRequiresLengthFinish(t *testing.T) {
	...
	if issue, _ := DetectIssue(cfg, loop, "stop", false); issue == IssueRepetition {
	...
	if issue, _ := DetectIssue(cfg, loop, "length", false); issue != IssueRepetition {
...
// TestDetectIssueEmptyContent is the regression test for a confirmed
// real-world failure: a completion with blank content and finish_reason
// "stop" previously passed through as IssueNone ("clean"), so neither the
// retry loop nor alert-continuation ever noticed — the client just got
// nothing and showed its own generic "please resend" error.
func TestDetectIssueEmptyContent(t *testing.T) {
	cfg := &DetectionConfig{RepetitionNgramSize: 6, RepetitionMinRepeats: 3, RepetitionWindowWords: 96, RepetitionRequiresLengthFinish: true}

	for _, blank := range []string{"", "   ", "\n\t \n"} {
		if issue, _ := DetectIssue(cfg, blank, "stop", false); issue != IssueEmpty {
			t.Errorf("DetectIssue(%q, \"stop\", false) = %q, want IssueEmpty", blank, issue)
		}
	}
}

// TestDetectIssueEmptyContentSkippedForToolCalls verifies a tool-calls-only
// turn (legitimately empty visible content — the "response" is the tool
// call itself) is never flagged as IssueEmpty.
func TestDetectIssueEmptyContentSkippedForToolCalls(t *testing.T) {
	cfg := &DetectionConfig{RepetitionNgramSize: 6, RepetitionMinRepeats: 3, RepetitionWindowWords: 96, RepetitionRequiresLengthFinish: true}
	if issue, _ := DetectIssue(cfg, "", "tool_calls", true); issue == IssueEmpty {
		t.Error("a tool-calls-only turn must not be flagged as IssueEmpty")
	}
}

// TestDetectIssueNonEmptyContentUnaffected verifies ordinary non-empty
// content is untouched by the new empty check.
func TestDetectIssueNonEmptyContentUnaffected(t *testing.T) {
	cfg := &DetectionConfig{RepetitionNgramSize: 6, RepetitionMinRepeats: 3, RepetitionWindowWords: 96, RepetitionRequiresLengthFinish: true}
	if issue, _ := DetectIssue(cfg, "a perfectly normal response", "stop", false); issue != IssueNone {
		t.Errorf("issue = %q, want IssueNone for ordinary content", issue)
	}
}
