...
// the first issue found (repetition > truncation > syntax, in that order)
// plus a human-readable detail of *what* triggered it — for repetition,
// the exact n-gram that repeated. The detail exists to make false
// positives diagnosable from the logs: without it, a retry log only says
// "repetition happened", with no way to tell genuine degeneration apart
// from legitimately repeated structure (XML tool tags, file paths,
// boilerplate) tripping the detector on healthy output.
func DetectIssue(cfg *DetectionConfig, content string, finishReason string) (Issue, string) {
